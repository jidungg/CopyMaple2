...
void CBayar::Priority_Update(_float fTimeDelta)
{
    ...
    m_vMoveDirectionXZ*=m_fRunSpeed* fTimeDelta;

    //공격 쿨이 안됐으면 그냥 계속 다가가기.
    //이동은 애니메이션이 끝나고 범위 밖에 있을 떄.
    //회전은 애니메이션이 끝났을 때. 

    //공격 범위 안이 될 때 까지 이동.
    //공격 범위 안이면 멈춤.
    //추적 범위 밖으로 나가면 다시 공격범위 안이 될 때 까지 이동.
    //이동을 멈춰야 되는 상황 : 공격 애니메이션 중임. 공격 범위 안에 들어온 후 추적범위 밖으로 나가지 않았음.
    //회전을 멈춰야 되는 상황 : 공격 애니메이션 중임.

    // 공격 범위 안임
    if (m_fTargetDistance <= m_fAttackRange)
    {
        m_bChase = false; //추적 범위 밖으로 나갈 때 까지 추적하지 않음.
        if (false == m_bAttack&&Is_AttackCoolReady())//공격 범위 안인데 공격 쿨도 다 찼음 -> 공격
        {
            m_bAttack = true;
            cout << "Attack!!\n";
    ...
    //공격 범위 밖이고 추적 범위 밖임
	else if (m_fTargetDistance > m_fChaseRange) 
	{
		m_bChase = true; // 추적 시작
	}
...
_bool CBayar::Check_Collision(CGameObject* pOther)
{
    ...
    case Client::LAYER_PLAYER:
    {
        ...
        if (false == (m_bDetected = m_vecCollider[DETECTION]->Intersect(vPos)))
        {
			m_pTarget = nullptr;
            return false;
        }
